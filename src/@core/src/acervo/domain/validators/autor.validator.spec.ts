import { AutorValidator, AutorRules, AutorValidatorFactory } from '#acervo/domain/validators/autor.validator';


describe("CategoryValidator Tests", () => {
  let validator: AutorValidator;

  beforeEach(() => (validator = AutorValidatorFactory.create()));
  ...
  test("casos válidos de todos os campos", () => {
    ...
    arrange.forEach((item) => {
      ...
      expect(validator.validatedData).toStrictEqual(new AutorRules(item));
    });
  });
});
